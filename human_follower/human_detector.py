...
class HumanDetector(Node):
    ...
    def scan_callback(self, msg):
        ...
        # Detect nearest human based on leg pairs
    ...
    def find_human(self, legs):
        """ Identifies the nearest human position based on leg pairs. """
        if len(legs) < 2:  # Need at least two legs to detect a human
            return None

        # Store potential human positions and their distances
        human_positions = []
        distances = []

        # Check all pairs of legs
        for i in range(len(legs)):
            for j in range(i + 1, len(legs)):
                center_i = np.mean(legs[i], axis=0)
                center_j = np.mean(legs[j], axis=0)
                dist_between_legs = np.linalg.norm(center_i - center_j)
                if dist_between_legs < self.max_leg_dist:
                    # Calculate potential human position as midpoint
                    human_pos = (center_i + center_j) / 2
                    # Calculate distance from origin (0,0) in laser frame
                    distance_to_robot = np.linalg.norm(human_pos)
                    human_positions.append(human_pos)
                    distances.append(distance_to_robot)

        if not human_positions:
            return None

        # Find the index of the nearest human
        nearest_idx = np.argmin(distances)
        nearest_human_pos = human_positions[nearest_idx]
        
        self.get_logger().info(f"Nearest human detected at distance: {distances[nearest_idx]:.2f} meters")
        if distances[nearest_idx] > self.max_distance_to_human:
            self.get_logger().warn(f"Human too far away, ignoring")
            return None
